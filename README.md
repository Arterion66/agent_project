...
I have decided to use as model api: https://fireworks.ai
The advantage is that it is free and you can create accounts if needed.

I have created the graph: agent_pokemon that consists of an agent that returns pokemon data accessing to an api and if you want that same agent can return the wiki of the pokemon and on the other hand an agent that gives of his knowledge a few data.

I have also created another graph: **agent_quotes**, this is the main one, 
this one has also **2 agents**, one in which you can ask for dates to book appointments, reschedule them, reschedule them and cancel them, the other agent is in charge of sending an email in case an appointment has been booked.
For this you will need to deploy the docker compose that is in project_quotes, this will raise a postgres, pgadmin and a docker that I have created for the management of basic appointments.
